# 그래프 레퍼런스 UX 요구사항 인벤토리

> 상태: **Approved — 사용자가 그래프 관련 데스크톱 범위를 승인함**
> 문서 유형: 레퍼런스 기반 요구사항 인벤토리
> 작성일: 2026-08-30
> 구현 계약: 사용자가 선택한 `agentic-execution`으로 데스크톱 범위를 구현한다.

이 문서는 [AI Coding Dictionary](https://www.aicodingdictionary.com/)를 그래프 상호작용과 선택 상세 콘텐츠의 참고로만 기록한다. 기존 승인 문서인 [레퍼런스 정렬 그래프 UX 통합 명세](./graph-reference-ux-spec.md)는 수정하지 않으며, 이 문서는 그 명세를 보완하는 데스크톱 구현 요구사항이다. 브랜드 문구, 출처 크레딧, 사이트 전용 header/chrome, loading splash, About·palette·sound 설정은 제품 UI 범위에서 제외한다. 사용자는 2026-08-30에 그래프 관련 데스크톱 범위를 승인했다.
...
| --- | --- | --- | --- | --- |
| `loading` | 최초 진입 또는 새 route 로드 | 그래프가 준비되기 전 비어 있거나 숨겨짐 | 기존 제품의 비브랜디드 로딩 상태만 사용 | 준비가 끝나면 `idle` 또는 deep-link 선택 상태 |
| `idle` | root 진입, 선택 해제 | 3D 노드와 모든 label이 viewport 전체에 분산; label은 낮은 대비; 관계선은 보이지 않음; ambient drift 지속 | 기존 제품 controls만 유지; reference 전용 header/chrome 없음 | `https://www.aicodingdictionary.com/` |
...
- 수용 기준: 같은 node를 canvas, 상세 패널, deep-link, copy markdown에서 식별할 수 있고, 화면 surface마다 이름이 달라지지 않는다.

#### AH-DATA-002 — section과 순번

- 우선순위: MUST
- 관찰됨: 선택 패널은 section 이름과 `현재 순번 / 전체 69`를 함께 표시한다.
- 수용 기준: 전체 term 수가 150개로 늘어도 순번은 deterministic하며, Prev/Next가 같은 순서를 사용한다.
...
- 우선순위: MUST
- 관찰됨: 선택 term의 원문 링크가 새 탭으로 열린다(`target="_blank"`, `rel="noopener noreferrer"`).
...
#### AH-ROUTE-005 — external navigation isolation

- 우선순위: MUST
- 관찰됨: source link는 새 탭 외부 이동이다.
- 수용 기준: 외부 링크 클릭이 local `term` history를 변경하지 않는다.
...
- 우선순위: MUST
- 관찰됨: Close, relation pill, pager, copy/share에 accessible name이 있다.
- 수용 기준: canvas 자체의 시각 정보에 의존하지 않고, 모든 주요 action을 이름 있는 control과 semantic article로 노출한다.

#### AH-A11Y-002 — keyboard Escape

- 우선순위: MUST
- 관찰됨: Escape가 selected detail을 닫는다.
- 수용 기준: Escape가 선택 상세를 닫고 focus가 그래프 surface로 돌아간다.

#### AH-A11Y-003 — keyboard traversal

- 우선순위: MUST
- 관찰됨: DOM에 named button, complementary role, semantic article이 있으나, 본 조사 backend에서는 안정적인 Tab 순서를 계측하지 못했다.
- 수용 기준: keyboard-only 사용자가 그래프 controls → detail controls → rich inline links 순서로 이동할 수 있고, focus ring이 noise canvas에서 사라지지 않는다.
...
- 수용 기준: 사용자가 reduced-motion 지원을 승인하면 ambient drift, hover attraction, camera transition, rail animation을 줄이는 명시적 정책과 브라우저 증거를 추가한다.
...
- source external link와 copy/share action의 semantics
...
아래 항목은 조사 당시 미확인이었지만, 사용자가 그래프 관련 데스크톱 구현을 승인하면서 기능을 막지 않는 보수적 기본값으로 확정했다. 수치·스타일은 기존 제품 방향을 유지하고, 새 서버·저장소·런타임은 추가하지 않는다.

1. Share는 지원 브라우저에서 native share를 사용하고, 미지원 환경에서는 현재 term link를 clipboard로 복사하며 결과 상태를 알린다.
2. `prefers-reduced-motion: reduce`는 ambient drift·hover attraction·camera transition·rail CSS animation을 즉시 감쇠하거나 중지한다.
3. graph edge의 정확한 곡률·점선 간격·색상·속도는 기능 계약이 아닌 제품 스타일로 보고, hover dashed child가 canonical geometry를 공유하는 안전한 구현을 사용한다.

## 8. 승인 후 적용 순서와 완료 증거

승인 후 아래 순서로 적용했고, 각 단계는 브라우저 증거를 얻은 뒤 다음 단계로 넘어갔다.

1. 콘텐츠/route contract: node schema, section/order, deep-link/history.
...
4. graph accessibility: semantic fallback, keyboard controls, no-branded overlay.
5. 150-node desktop verification: no-flicker, no-re-layout, reduced-motion decision.

현재 문서는 **승인된 그래프 관련 데스크톱 구현 요구사항**이며, 위 수직 슬라이스는 구현 완료 상태다. 모바일·태블릿·터치 전용 동작은 계속 범위 밖이다.
