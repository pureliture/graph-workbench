...
function labelVisibilityForNode(
  node: RenderNode,
...
function makeCameraFacingMesh(mesh: Mesh): void {
...
function pointOnQuadraticCurve(
  ...
  const curve = Math.min(
    DEFAULT_LINK_CURVE.maximumPlanarOffset,
    planarDistance * DEFAULT_LINK_CURVE.planarDistanceRatio,
  );
...
function cameraTransitionDuration(
  options: GraphCameraTransitionOptions,
  fallback: number,
): number {
...
interface SceneNodeTransition {
  ...
  readonly targetBodyVisible: boolean;
...
function hostRecoveryKeyForData(data: RenderGraphData | null): string | null {
  return data?.presentation.recoveryKey ?? null;
}

function isCameraPose(value: GraphTransitionObservation["camera"]): value is NonNullable<GraphTransitionObservation["camera"]> {
  if (!value) return false;
  return [
    value.position.x,
    value.position.y,
...
interface NodeDensityVisibility {
  readonly bodyVisible: boolean;
  readonly labelVisible: boolean;
}

const FULL_NODE_DENSITY_VISIBILITY: NodeDensityVisibility = Object.freeze({
  bodyVisible: true,
  labelVisible: true,
});
...
function sceneVisualForNode(
  ...
  density: NodeDensityVisibility = FULL_NODE_DENSITY_VISIBILITY,
): NodeSceneVisual {
...
export function createThreeForceGraphRenderer({
  ...
  let rendererViewport: { width: number; height: number } | null = null;
  ...
  let focusCameraBaseline: CameraPose | null = null;
  let activityState: GraphActivityState = {};
  let explicitlySuspended = false;
  let zeroSized = container.clientWidth <= 0 || container.clientHeight <= 0;
  let webglContextLost = false;
  let webglRecoveryCapsule: GraphRecoveryCapsule | null = null;
  const privateRecoveryRevisions = new WeakMap<GraphRecoveryCapsule, string>();
  const ambientNodes = new Map<string, AmbientNodeState>();
  const ambientLinks = new Map<string, AmbientLinkState>();
  let densityVisibilityByNodeId = new Map<string, NodeDensityVisibility>();
  ...
  function densityVisibilityForNode(nodeId: string): NodeDensityVisibility {
  ...
  function refreshDensityVisibility(force = false): void {
    ...
    const candidates = data.nodes.map((node) => {
      ...
      const selectedDistance = position && selectedState
        ? Math.hypot(
          position.x - selectedState.renderedX,
          position.y - selectedState.renderedY,
          position.z - selectedState.renderedZ,
        )
        : 0;
      return {
        cameraDistance,
    ...
    const backgroundCandidates = candidates
      ...
      .sort((left, right) => {
        ...
        if (selectedNodeId !== null) {
          ...
          if (Math.abs(selectedDifference) > AMBIENT_VISUAL_EPSILON) return selectedDifference;
        }
        const centerDifference = left.screenCenterDistance - right.screenCenterDistance;
        if (Math.abs(centerDifference) > AMBIENT_VISUAL_EPSILON) return centerDifference;
        const cameraDifference = left.cameraDistance - right.cameraDistance;
        if (Math.abs(cameraDifference) > AMBIENT_VISUAL_EPSILON) return cameraDifference;
        return left.id.localeCompare(right.id);
      });
    const selectBackground = (
      budget: number,
      cell: { readonly height: number; readonly width: number },
      candidatePool = backgroundCandidates,
    ): ReadonlySet<string> => {
      const selected = new Set<string>();
      const occupiedCells = new Set<string>();
      const selectedCandidates: typeof backgroundCandidates[number][] = [];
      const center = { x: viewport.width / 2, y: viewport.height / 2 };
      while (selected.size < budget) {
        let best: typeof backgroundCandidates[number] | null = null;
        let bestScore = -Infinity;
        for (const candidate of candidatePool) {
          if (selected.has(candidate.id) || !candidate.inViewport || !candidate.projected) continue;
          ...
          const separation = selectedCandidates.length === 0
            ...
            : Math.min(...selectedCandidates.map((selectedCandidate) => (
              Math.hypot(candidate.x - selectedCandidate.x, candidate.y - selectedCandidate.y)
            )));
          if (
            separation > bestScore + AMBIENT_VISUAL_EPSILON
            || (
    ...
    const bodyBackground = bodiesNeedCulling
      ? selectBackground(
        Math.max(0, (compact ? DENSITY_VISIBILITY.bodyBudget.compact : DENSITY_VISIBILITY.bodyBudget.regular)
          - protectedNodeIds.size),
        compact ? { height: 38, width: 38 } : { height: 50, width: 50 },
      )
    ...
    const labelBackground = labelsNeedCulling
      ? selectBackground(
        Math.max(0, (compact ? DENSITY_VISIBILITY.labelBudget.compact : DENSITY_VISIBILITY.labelBudget.regular)
          - protectedNodeIds.size),
        compact ? DENSITY_VISIBILITY.labelCell.compact : DENSITY_VISIBILITY.labelCell.regular,
        labelCandidates,
      )
  ...
  function refreshAmbientLinkObject(state: AmbientLinkState, object: Object3D | null): void {
    ...
    state.lastOpacity = Number.NaN;
    state.lastWidth = Number.NaN;
    if (state.object) state.object.visible = state.link.visual.visible;
  ...
  function ensureAmbientTransparency(
    material: MeshBasicMaterial | MeshStandardMaterial | LineBasicMaterial,
    transparent: boolean,
  ): void {
      ...
      const readableLabelScale = interactionLabelMinimumPixels > 0
        ? Math.max(
          ...
          labelScaleForMinimumPixels(
            labelDistance,
            (state.defaultVisual?.baseLabelScale.y ?? 8) * LABEL_VISIBLE_GLYPH_HEIGHT_RATIO,
            scale,
            projection,
  ...
  function applyFocusedLinkFlow(): void {
    ...
    for (const state of ambientLinks.values()) {
      ...
      state.active = Boolean(
        linkVisible && (incident || idleFlow) && state.object && ambientMotionEnabled(),
      );
  ...
  function applyAmbientVisuals(): void {
    ...
    if (!pendingSceneTransition && !activeTransition?.scene) applyCameraRelativeDepth();
  ...
  function applyFinalVisuals(data: RenderGraphData): void {
    ...
    data.nodes.forEach((node) => {
      ...
      applyNodeVisual(
        ...
        visual.bodyVisible,
        visual.labelVisible,
        lightQuietContext && visual.labelVisible
  ...
  function applySceneFrame(scene: SceneTransition, progress: number, final = false): void {
    ...
    scene.nodes.forEach((transition) => {
      ...
      applyNodeVisual(
        ...
        final ? transition.targetLabelVisible : true,
        final
  ...
  function refreshMotionPauseState(): void {
    ...
    if (nextPaused) {
      ...
      if (pauseChanged) applyAmbientVisuals();
      return;
    }
  ...
  const updateCameraControlInteraction = () => {
    ...
    if (cameraControlInteractionActive && !activeTransition) cancelCameraTransition();
  ...
  function captureRecoveryCapsule(): GraphRecoveryCapsule | null {
  ...
  function restoreRecoveryCapsule(capsule: GraphRecoveryCapsule): boolean {
  ...
  const onWebGLContextRestored = () => {
    ...
    if (capsule) restoreRecoveryCapsule(capsule);
    refreshMotionPauseState();
  };
  ...
  function fullGraphCameraTarget(): CameraPose | null {
    const data = currentData;
    if (!data) return null;
  ...
  function transitionToNode(nodeId: string, options: GraphCameraTransitionOptions): void {
    ...
    if (!options.reducedMotion && activeScene?.targetFocusNodeId === nodeId) {
      ...
      if (targetCamera && activeTransition) activeTransition.targetCamera = targetCamera;
      return;
    }
    if (activeTransition) cancelCameraTransition();
    const targetCamera = nodeCameraTarget(nodeId);
  ...
  function restoreFocusCamera(options: GraphCameraTransitionOptions): void {
    pendingSceneTransition = null;
