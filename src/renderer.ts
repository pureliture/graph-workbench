...
export function createThreeForceGraphRenderer({
  ...
  let hoverAttractionFocusNodeId: string | null = null;
  ...
  let hoverAttractionLastElapsedMs: number | null = null;
  ...
  function refreshDensityVisibility(force = false): void {
    ...
    if (data.nodes.length < DENSITY_VISIBILITY.labelCullingAtNodeCount) {
      ...
      densityVisibilityByNodeId = new Map(
        data.nodes.map((node) => [node.id, FULL_NODE_DENSITY_VISIBILITY] as const),
      );
      return;
    }
    ...
    const candidates = data.nodes.flatMap((node) => {
      ...
      if (![projected.x, projected.y].every(Number.isFinite)) return [];
    ...
    const nextVisibility = new Map<string, NodeDensityVisibility>(
      data.nodes.map((node) => [node.id, { bodyVisible: true, labelVisible: false }] as const),
    );
    const occupiedLabels: Array<{ readonly x: number; readonly y: number }> = [];
    const nearOccupiedLabel = (x: number, y: number): boolean => occupiedLabels.some((occupied) => (
      Math.abs(occupied.x - x) < DENSITY_VISIBILITY.labelMinimumHorizontalGap
      && Math.abs(occupied.y - y) < DENSITY_VISIBILITY.labelMinimumVerticalGap
    ));
    const reserveLabel = (x: number, y: number): void => {
    ...
    for (const candidate of candidates) {
      ...
      if (!insideViewport && !candidate.alwaysVisible) continue;
      ...
      if (insideViewport) reserveLabel(candidate.x, candidate.y);
    }
  ...
  function hoverLineFor(state: AmbientLinkState): Line | null {
    return state.hoverLine;
  }

  function activeHoverNodeId(): string | null {
    if (hoverNodeId !== null) return hoverNodeId;
    return hoverAttractionProgress > 0 ? hoverAttractionFocusNodeId : null;
  }

  function applyHoverLineVisual(
    state: AmbientLinkState,
    visible: boolean,
    opacity: number,
    width: number,
  ): void {
    ...
    if (!line) return;
    if (line.visible !== visible) line.visible = visible;
    const material = line.material;
    if (!(material instanceof LineDashedMaterial)) return;
    if (changedAmbientVisualValue(material.opacity, opacity)) material.opacity = opacity;
    if (changedAmbientVisualValue(material.linewidth, width)) material.linewidth = width;
    ensureAmbientTransparency(material, true);
  }
  ...
  function updateAmbientNodePositions(): void {
    ...
    if (hoverNodeId !== null) hoverAttractionFocusNodeId = hoverNodeId;
    ...
    if (hoverAttractionProgress !== hoverAttractionTarget) {
      ...
      const transitionDelta = elapsedDelta > 0
        ...
        : hoverAttractionTransitionElapsedMs === 0
          ? HOVER_ATTRACTION_FRAME_START_MS
          : 0;
      hoverAttractionTransitionElapsedMs = Math.min(
        durationMs,
        hoverAttractionTransitionElapsedMs + transitionDelta,
      );
      ...
      hoverAttractionProgress = interpolate(
        hoverAttractionTransitionStart,
        hoverAttractionTarget,
        easedProgress,
      );
      if (transitionProgress >= 1) hoverAttractionProgress = hoverAttractionTarget;
    }
    ...
    if (hoverFocus && currentData) {
      currentData.links.forEach((link) => {
        if (link.source === hoverFocus.id) hoverNeighborIds.add(link.target);
        if (link.target === hoverFocus.id) hoverNeighborIds.add(link.source);
      });
    }
      ...
      if (state.object?.userData.graphDefaultNodeObject === true) {
        ...
        if (hoverFocus && state.id !== hoverFocus.id && hoverAttractionProgress > 0) {
          ...
          const appliedDistance = Math.min(
            HOVER_ATTRACTION_MAX_OFFSET,
            distance * factor * hoverAttractionProgress,
          );
  ...
  function applyCameraRelativeDepth(): void {
    ...
    for (const state of ambientNodes.values()) {
      ...
      applyAmbientDefaultNodeVisual(
        ...
        scale,
        density.bodyVisible,
        density.labelVisible,
        density.labelVisible ? readableLabelOpacity : 0,
        labelScale,
      );
    }
  }
  ...
  function applyFocusedLinkFlow(): void {
    ...
    for (const state of ambientLinks.values()) {
      ...
      state.active = Boolean(
        linkVisible && ((selectedFocus && incident) || idleFlow) && state.object && ambientMotionEnabled(),
      );
      ...
      if (hoverOnly) {
        ...
        applyHoverLineVisual(
          state,
          hoverOpacity > 0.02,
          hoverOpacity,
          Math.max(1.05, state.baseWidth),
        );
        continue;
      }
      applyHoverLineVisual(state, false, 0, state.baseWidth);
